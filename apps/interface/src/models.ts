...
export class QueuedPrompt {
  ...
  constructor(
    params: Partial<Omit<QueuedPrompt, 'addLog' | 'canRetry' | 'shouldExecuteNow'>> = {},
  ) {
...
// QueueState é usado apenas como DTO de leitura pelo CLI
export class QueueState {
  ...
  getStats(): QueueStats {
    ...
    for (const status of Object.values(PromptStatus)) {
      if (status === PromptStatus.COMPLETED) {
        statusCounts[status] = this.totalProcessed;
      } else if (status === PromptStatus.FAILED) {
        statusCounts[status] = this.failedCount;
      } else {
        statusCounts[status] = this.prompts.filter((p) => p.status === status).length;
      }
    }
    ...
    return {
      ...
      lastProcessed: this.lastProcessed ? this.lastProcessed.toISOString() : null,
      currentRateLimit: {
        isRateLimited: this.currentRateLimit?.isRateLimited ?? false,
        resetTime: this.currentRateLimit?.resetTime?.toISOString() ?? null,
      },
    };
  }
}
